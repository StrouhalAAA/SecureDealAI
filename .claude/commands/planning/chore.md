...
- Create the plan in the `specs/*.md` file. Name it appropriately based on the `Chore` (e.g., `specs/chore-cleanup-unused-imports.md`).
...
- `ADWS/*.py` contain astral uv single file python scripts. So if you want to run them use `uv run ADWS/<script_name>`.
- When you finish creating the plan for the chore, follow the `Report` section to properly report the results of your work.

## Relevant Files

Focus on the following files based on chore type:

**Frontend chores:**
- `apps/web/src/` - Vue 3 components, composables, pages
- `apps/web/src/components/` - Reusable UI components
- `apps/web/src/composables/` - Vue composables (useAuth, useDetailData, etc.)
- `apps/web/src/pages/` - Route pages (Dashboard, Detail, etc.)
...
Ignore test files unless the chore specifically involves testing.
...
- `npm run test:e2e` - Run Playwright E2E tests (if UI affected)
